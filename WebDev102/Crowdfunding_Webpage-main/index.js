...
const numOfGames = GAMES_JSON.reduce((accumulator) => (accumulator += 1), 0);
gamesCard.innerHTML = `${numOfGames.toLocaleString()}`;
...
function filterUnfundedOnly() {
  ...
  // returns length for us to log
  console.log("Length : " + unfundedGames.length);
}
...
const unfundedGamesString = `There ${
  unfundedGamesCount === 1 ? "is" : "are"
} ${unfundedGamesCount} ${
  unfundedGamesCount === 1 ? "game" : "games"
} currently unfunded.`;
...
firstGameElement.textContent = `The game with the highest pledge is ${firstGame.name} with ${firstGame.pledged} pledges.`;

// do the same for the runner up item
const secondGameElement = document.createElement("p");
secondGameElement.textContent = `The runner-up game is ${secondGame.name} with ${secondGame.pledged} pledges.`;
